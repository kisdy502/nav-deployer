...
/** 点位图层：圆盘 + 朝向箭头 + 选中环 + 旋转手柄 + CSS2D 名称标签 */
export class PointLayer {
  readonly group = new THREE.Group()
  private visuals = new Map<number, PointVisual>()

  sync(points: NavPointVO[], selectedId: number | null) {
  ...
  private create(p: NavPointVO): PointVisual {
    ...
    const disc = new THREE.Mesh(new THREE.CircleGeometry(0.22, 32), discMat)
    disc.position.z = 0.01
    disc.userData.pick = { type: 'point', id: p.id }
    const arrow = new THREE.Mesh(
      new THREE.ConeGeometry(0.09, 0.3, 12),
      new THREE.MeshBasicMaterial({ color: 0x303133 }),
    )
  ...
  private setArrow(v: PointVisual, yaw: number) {
    ...
    v.handle.position.x = Math.cos(yaw) * 0.55
    v.handle.position.y = Math.sin(yaw) * 0.55
  }
